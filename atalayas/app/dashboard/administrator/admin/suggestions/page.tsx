  ...
  const handleSubmitSuggestion = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(API_ROUTES.SUGGESTIONS.CREATE, {
        ...
        body: JSON.stringify({ title: newTitle, content: newContent, targetRole: "GENERAL_ADMIN", authorId: user?.id, authorRole: "ADMIN" }),
      });
      if (res.ok) { setNewTitle(""); setNewContent(""); fetchData(); }
  ...
  const handleRespond = async (status: SuggestionStatus) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSelected(null); setResponseBody(''); fetchData();
    ...
    <div className="flex min-h-screen bg-background font-sans text-foreground transition-colors duration-300">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex bg-muted/50 p-1 rounded-xl border border-border/50">
              {(['RECEIVED', 'SENT'] as const).map((v) => (
                <button key={v} onClick={() => setView(v)}
                  className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${view === v ? 'bg-card text-primary shadow-sm' : 'text-muted-foreground'}`}
                >
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {view === 'RECEIVED' ? (
            <>
              <div className="px-8 py-4 flex gap-2 border-b border-border/40 bg-card/50">
              ...
              <div className={`flex-1 grid transition-all duration-500 ${selected ? 'lg:grid-cols-[1fr_450px]' : 'grid-cols-1'}`}>
                <div className="overflow-y-auto p-8 space-y-3 no-scrollbar bg-background">
                  {loading ? <p className="text-center py-10 opacity-50">Cargando...</p> : filteredSuggestions.map((s) => (
                    <div key={s.id} onClick={() => setSelected(s)} className={`p-5 rounded-[22px] border cursor-pointer transition-all ${selected?.id === s.id ? 'bg-card border-primary shadow-md' : 'bg-card border-border/50 hover:border-primary/50'}`}>
                ...
                {selected && (
                  <div className="bg-card border-l border-border/60 p-8 overflow-y-auto animate-in slide-in-from-right-5 shadow-2xl">
                    ...
                    <h2 className="text-xl font-black mb-4 text-foreground">{selected.title}</h2>
                    <div className="bg-muted/30 p-6 rounded-3xl border border-border/40 mb-8 italic text-sm text-foreground/80 font-medium leading-relaxed">
                        "{selected.content}"
                    </div>
                    {selected.status === 'PENDING' ? (
                      <div className="space-y-4 pt-6 border-t border-border/40">
                        <textarea value={responseBody} onChange={e => setResponseBody(e.target.value)} placeholder="Escribe una respuesta para el empleado..." className="w-full p-4 bg-background border border-border/60 rounded-2xl text-sm h-32 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-foreground" />
                        <div className="flex gap-2">
                          <button onClick={() => handleRespond('ACCEPTED')} className="flex-1 py-3 bg-emerald-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-600 transition-colors shadow-lg shadow-emerald-500/20">Aceptar</button>
                          <button onClick={() => handleRespond('REJECTED')} className="flex-1 py-3 bg-destructive/10 text-destructive border border-destructive/20 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-destructive hover:text-white transition-all">Rechazar</button>
                        </div>
                      </div>
                    ) : (
                      <div className={`p-5 rounded-2xl border ${statusConfig[selected.status].bgClass}`}>
                          <p className="text-xs font-bold uppercase opacity-50 mb-2 tracking-tighter">Respuesta oficial:</p>
                          <p className="text-sm italic font-medium">"{selected.response}"</p>
                      </div>
                    )}
                  </div>
                )}
            ...
            <div className="flex-1 grid lg:grid-cols-[400px_1fr] overflow-hidden">
              <div className="p-8 border-r border-border/40 bg-card overflow-y-auto">
                <h3 className="font-black text-lg mb-6 text-foreground">Nueva Sugerencia</h3>
                <form onSubmit={handleSubmitSuggestion} className="space-y-5">
                  <input placeholder="Asunto" value={newTitle} onChange={e => setNewTitle(e.target.value)} className="w-full p-4 bg-background border border-border/60 rounded-2xl text-sm font-bold focus:ring-1 focus:ring-primary/30 outline-none text-foreground" required />
                  <textarea placeholder="Explica tu idea o queja detalladamente..." value={newContent} onChange={e => setNewContent(e.target.value)} className="w-full p-4 bg-background border border-border/60 rounded-2xl text-sm min-h-50 focus:ring-1 focus:ring-primary/30 outline-none text-foreground" required />
                  <button disabled={isSubmitting} className="w-full py-4 bg-primary text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] disabled:opacity-50 shadow-xl shadow-primary/20">{isSubmitting ? 'Enviando...' : 'Enviar a Atalayas EGM'}</button>
                </form>
              </div>
              <div className="p-8 overflow-y-auto bg-muted/10">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Mis Envíos</h3>
                  <div className="flex gap-1.5 bg-card p-1 rounded-xl border border-border/40">
                    {(['ALL', 'PENDING', 'ACCEPTED', 'REJECTED'] as const).map((f) => (
                ...
                <div className="space-y-4">
                  {loading ? <p>Cargando...</p> : filteredSuggestions.map((s) => (
                    <div key={s.id} className="group bg-card border border-border/40 rounded-[28px] p-6 shadow-sm relative transition-all hover:border-primary/20">
                      ...
                      {s.response && <div className="mt-4 pt-4 border-t border-border/30"><p className="text-xs font-bold italic opacity-80 text-foreground/70">"{s.response}"</p></div>}
                    </div>
                  ))}
                </div>
              </div>
