...
export default function AdminContentDetail() {
    ...
    const fetchContent = async () => {
      ...
      try {
        const res = await fetch(
          API_ROUTES.CONTENT.GET_BY_ID(params.contentId),
          {
            headers: {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(
        API_ROUTES.CONTENT.GET_BY_ID(params.contentId as string),
        {
          method: "PATCH",
          headers: {
              ...
              {isEditing ? (
                <div className="space-y-5">
                  <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm space-y-4">
                    <p className="text-[11px] font-black uppercase tracking-[0.15em] text-[#86868b]">
                      Texto e Imagen
                    </p>
                  ...
                  <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm space-y-4">
                    <p className="text-[11px] font-black uppercase tracking-[0.15em] text-[#86868b]">
                      Documento PDF (Material de estudio)
                    </p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <input
                        value={formData.url}
                        onChange={(e) => set("url", e.target.value)}
                        placeholder="Link directo al PDF..."
                        className={inputClass}
                      />
                  ...
                  {(formData.podcast || formData.quiz) && (
                    <div className="bg-[#f5f5f7] p-6 rounded-[2rem] space-y-4 opacity-80">
                      <p className="text-[10px] font-black uppercase text-[#86868b]">
                        Datos Generados por IA (Solo lectura)
                      </p>
                      <div className="grid grid-cols-2 gap-3">
                        {formData.podcast && (
                          <div className="p-3 bg-white rounded-xl text-[10px] font-mono overflow-hidden h-20">
                            Podcast:{" "}
                            {JSON.stringify(formData.podcast).substring(0, 50)}
                            ...
                          </div>
                        )}
                        {formData.quiz && (
                          <div className="p-3 bg-white rounded-xl text-[10px] font-mono overflow-hidden h-20">
                            Quiz:{" "}
                            {JSON.stringify(formData.quiz).substring(0, 50)}...
                          </div>
                        )}
                      </div>
                    </div>
              ...
              <aside className="action-box space-y-4">
                {content.url && (
                  <div className="bg-white p-6 rounded-[2rem] border border-black/5 shadow-sm text-center">
                    ...
                    <p className="text-xs font-bold text-[#86868b] uppercase mb-4">
                      Material PDF
                    </p>
                    <a
                      href={content.url}
                      target="_blank"
                      className="block w-full py-3 bg-[#1d1d1f] text-white rounded-xl text-xs font-bold hover:bg-black transition-all"
                    >
                      Abrir documento
                    </a>
                  </div>
                ...
                {content.podcast && (
                  <div className="bg-[#0071e3] p-6 rounded-[2rem] text-white shadow-lg shadow-blue-500/20">
                    ...
                    <h4 className="text-sm font-bold mb-4 leading-tight">
                      Resumen disponible en podcast
                    </h4>
                    <button className="w-full py-2.5 bg-white/20 backdrop-blur-md rounded-lg text-xs font-bold hover:bg-white/30">
                      Reproducir
                    </button>
                  </div>
                )}
      ...
      {showDeleteModal && (
        ...
        >
          ...
          >
            ...
            <h2 className="text-2xl font-bold text-[#1d1d1f] mb-3 text-center">
              ¿Eliminar contenido?
            </h2>
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                disabled={deleting}
                className="w-full py-4 rounded-2xl font-bold bg-[#ff3b30] text-white"
              >
                {deleting ? "Eliminando..." : "Eliminar ahora"}
              </button>
              <button
                onClick={() => setShowDeleteModal(false)}
                className="w-full py-4 rounded-2xl font-semibold text-[#0071e3]"
              >
                Cancelar
              </button>
            </div>
