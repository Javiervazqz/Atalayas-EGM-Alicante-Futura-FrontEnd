...
export default function AdminContentDetail() {
  ...
  const removeQuestion = (qIdx: number) => {
    setFormData({
      ...formData,
      quiz: { questions: formData.quiz.questions.filter((_: any, i: number) => i !== qIdx) }
    });
  };
  ...
  useEffect(() => {
    if (activeTab === "lectura" && imageRef.current) {
      const zoomInstance = zoom(imageRef.current, { background: "rgba(0,0,0,0.9)", margin: 20 });
                ...
                {isEditing ? (
                  <div className="space-y-10">
                    <section className="space-y-6">
                      <h3 className="text-lg font-black border-l-4 border-primary pl-4 uppercase tracking-tighter">General</h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <input placeholder="URL Imagen" value={formData.imageUrl} onChange={(e) => setFormData({ ...formData, imageUrl: e.target.value })} className={inputClass} />
                        <input placeholder="URL Video" value={formData.videoUrl} onChange={(e) => setFormData({ ...formData, videoUrl: e.target.value })} className={inputClass} />
                      </div>
                      <label className={labelClass}>Cuerpo (Markdown)</label>
                      <textarea rows={10} value={formData.summary} onChange={(e) => setFormData({ ...formData, summary: e.target.value })} className={`${inputClass} font-mono text-xs`} />
                    </section>

                    <section className="space-y-8 pt-10 border-t border-border">
                      <div className="flex justify-between items-center">
                        <h3 className="text-lg font-black uppercase tracking-tighter text-orange-600">Evaluación</h3>
                        <button onClick={addQuestion} className="text-[9px] font-black uppercase bg-orange-500/10 text-orange-600 px-4 py-2 rounded-lg">+ Añadir</button>
                      </div>
                      <div className="space-y-6">
                        {formData.quiz.questions.map((q: any, qIdx: number) => (
                          <div key={qIdx} className="relative p-6 bg-muted/20 border border-border/50 rounded-2xl space-y-4">
                            <button onClick={() => removeQuestion(qIdx)} className="absolute top-4 right-4 text-muted-foreground hover:text-destructive"><i className="bi bi-trash3"></i></button>
                            ...
                            <select value={q.correctAnswer} onChange={(e) => updateCorrectAnswer(qIdx, e.target.value)} className={`${inputClass} border-emerald-500/30`}>
                  ...
                  <div className="animate-in fade-in duration-500">
                    {activeTab === "multimedia" && (
                      <div className="aspect-video rounded-2xl overflow-hidden bg-black shadow-2xl">
                        <video key={content?.videoUrl} controls className="w-full h-full object-cover"><source src={content?.videoUrl} type="video/mp4" /></video>
                      </div>
                    )}
                    {activeTab === "lectura" && (
                      <div className="space-y-8">
                        {content?.imageUrl && (
                          <img ref={imageRef} src={content.imageUrl} className="w-full aspect-video object-cover rounded-2xl shadow-lg border border-border/50" alt="Cover" />
                        )}
                        <div className="prose dark:prose-invert max-w-none">
                          <ReactMarkdown
                            components={{
                              h2: ({ ...props }) => <h2 className="text-xl md:text-2xl font-black text-foreground mt-8 mb-4" {...props} />,
                              p: ({ ...props }) => <p className="text-[15px] md:text-[17px] leading-[1.8] text-muted-foreground mb-6" {...props} />,
                              li: ({ ...props }) => <li className="flex items-start gap-3 text-[14px] md:text-[16px] text-muted-foreground mb-4"><span className="mt-2.5 w-2 h-2 rounded-full bg-primary/40 shrink-0" /><span {...props} /></li>,
                              blockquote: ({ ...props }) => <blockquote className="border-l-4 border-primary bg-muted/40 p-4 md:p-6 rounded-r-2xl my-8 italic" {...props} />,
                            }}
                          >
                            {content?.summary}
                          </ReactMarkdown>
                    ...
                    {activeTab === "evaluacion" && (
                      <div className="bg-orange-500/5 border border-orange-500/10 rounded-[2rem] p-8 md:p-12 text-center space-y-6">
                        <div className="w-14 h-14 md:w-16 md:h-16 bg-orange-500 text-white rounded-2xl flex items-center justify-center mx-auto shadow-xl"><i className="bi bi-patch-question text-2xl md:text-3xl"></i></div>
                        <h4 className="text-xl md:text-2xl font-black italic">¿Deseas probar tus conocimientos?</h4>
                        <button onClick={() => setShowQuizModal(true)} className="bg-orange-500 text-white px-8 md:px-10 py-3 md:py-4 rounded-xl text-xs font-black uppercase tracking-widest shadow-xl hover:scale-105 transition-all">Iniciar Autoevaluación</button>
                      </div>
                    )}
                  </div>
                )}
          ...
          */}
          ...
          ">
            ...
            {content?.url?.toLowerCase().includes(".pdf") && (
              <a href={content.url} target="_blank" className="group w-full p-4 bg-muted/50 border border-border rounded-xl flex items-center gap-4 hover:border-primary transition-all">
                ...
                <div className="text-left overflow-hidden">
                  <p className="text-[8px] font-black uppercase text-muted-foreground">PDF</p>
            ...
            {content?.practiceLab && (
              <button onClick={() => setShowLabModal(true)} className="group w-full p-4 bg-blue-500/5 border border-blue-500/10 rounded-xl flex items-center gap-4 hover:border-blue-500 transition-all text-left">
                <div className="w-10 h-10 bg-blue-500 text-white rounded-xl flex items-center justify-center shrink-0 shadow-lg"><i className="bi bi-controller text-xl"></i></div>
                <div className="overflow-hidden">
                  <p className="text-[8px] font-black uppercase text-blue-600">Simulador IA</p>
                  <p className="text-xs font-bold truncate">Práctica Interactiva</p>
                </div>
              </button>
            )}
            ...
            <div className="md:mt-auto pt-4 md:pt-6 border-t border-border">
              <button onClick={() => setShowDeleteModal(true)} className="w-full py-3 text-[9px] font-black uppercase text-destructive hover:bg-destructive/5 rounded-xl tracking-widest transition-all">Eliminar Unidad</button>
            </div>
          </aside>
        </div>
      </main>
      ...
      {showLabModal && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/95 backdrop-blur-xl md:p-4">
          <div className="bg-card border border-border md:rounded-[40px] w-full max-w-6xl h-full md:h-[90vh] relative overflow-hidden shadow-2xl flex flex-col">
            <button onClick={() => { setShowLabModal(false); setIsLabStarted(false); }} className="absolute top-4 right-4 z-[130] w-10 h-10 bg-white/10 hover:bg-destructive rounded-full flex items-center justify-center text-white transition-all shadow-lg"><i className="bi bi-x-lg"></i></button>
            <div className="flex-1 flex flex-col bg-slate-950 overflow-hidden">
              {!isLabStarted ? (
                <div className="flex-1 flex flex-col items-center justify-center text-center p-6 space-y-6">
                  <div className="w-16 h-16 md:w-20 md:h-20 bg-blue-500/20 text-blue-500 rounded-3xl flex items-center justify-center mx-auto shadow-2xl"><i className="bi bi-controller text-3xl md:text-4xl"></i></div>
                  <div>
                    <h3 className="text-xl md:text-2xl font-black text-white mb-2">{content?.practiceLab?.scenarioTitle || "Simulador"}</h3>
                    <p className="text-slate-400 max-w-md mx-auto text-sm">Resuelve el desafío interactivo para completar esta unidad.</p>
                  </div>
                  <button onClick={() => setIsLabStarted(true)} className="bg-blue-600 hover:bg-blue-500 text-white px-8 md:px-10 py-3 md:py-4 rounded-xl font-black uppercase text-xs tracking-widest shadow-lg transition-all active:scale-95">Empezar Práctica</button>
                </div>
              ) : (
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-3 md:p-4">
          <div className="bg-card border border-border rounded-[1.5rem] md:rounded-[2rem] p-5 md:p-8 max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
            <div className="flex justify-between items-center mb-6 md:mb-8 shrink-0">
              <h3 className="text-xl md:text-2xl font-black italic">Vista Previa Evaluación</h3>
              <button onClick={() => { setShowQuizModal(false); setIsCorrected(false); }} className="text-muted-foreground hover:text-foreground"><i className="bi bi-x-circle text-2xl"></i></button>
            </div>
            ...
            <div className="pt-4 md:pt-6 mt-4 md:mt-6 border-t border-border">
              <button onClick={() => setIsCorrected(true)} className="w-full bg-primary text-white py-3 md:py-4 rounded-xl font-black uppercase text-xs tracking-widest shadow-lg">Corregir</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-[1.5rem] md:rounded-[2rem] p-8 md:p-10 max-w-sm w-full text-center shadow-2xl">
            ...
            <div className="flex gap-3 mt-8">
              ...
              <button onClick={handleDelete} disabled={isDeleting} className="flex-1 py-3 rounded-xl bg-destructive text-white font-black uppercase text-[10px]">{isDeleting ? "Borrando..." : "Eliminar"}</button>
            </div>
          </div>
        </div>
      )}
