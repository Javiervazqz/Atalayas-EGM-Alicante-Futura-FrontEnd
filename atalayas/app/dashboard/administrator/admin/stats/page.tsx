...
// Gráfica de línea con área y tooltips (Adaptada para un solo color)
...
// ── Modal Histórico Gigante con Animación SVG ─────────────────────────────

function YearlyHistorySelector({ data, title, theme }: { data: TrendEntry[], title: string, theme: 'blue' | 'violet' }) {
        ...
        {isOpen && (
          ...
          >
            ...
            >
              ...
              {/* Contenedor Gráfico Animado */}
              <div className="flex-1 w-full mt-4 mb-6 relative">
                
                {/* Capa de Línea Animada (Clip Path SVG) */}
                <motion.div
...
// ── Página principal de Estadísticas de Empresa ───────────────────────────
      ...
      <main className="flex-1 overflow-y-auto no-scrollbar flex flex-col">
        ...
        <div className="p-6 lg:p-8 space-y-4 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            <KpiCard label="Empleados" value={ov?.totalUsers ?? 0} icon="bi-people-fill" color="bg-blue-500/10 text-blue-400" />
            <KpiCard label="Cursos Activos" value={ov?.totalCourses ?? 0} icon="bi-journal-bookmark-fill" color="bg-amber-500/10 text-amber-400" />
            ...
            <KpiCard label="Onboarding" value={`${onboarding.finished}/${onboarding.total}`} icon="bi-person-check-fill" color="bg-emerald-500/10 text-emerald-400" />
            <KpiCard label="Sugerencias" value={suggestions.pending} icon="bi-chat-left-dots-fill" color="bg-pink-500/10 text-pink-400" />
          </div>

          {/* ── SECCIÓN: EQUIPO Y ROLES ───────────────────────── */}
          <Section title="Equipo y Distribución" icon="bi-person-vcard-fill" iconColor="bg-blue-500/10 text-blue-400">
          ...
          {/* ── SECCIÓN: FORMACIÓN Y RENDIMIENTO ──────────────── */}
          <Section title="Formación Académica" icon="bi-mortarboard-fill" iconColor="bg-amber-500/10 text-amber-400">
          ...
          {/* ── SECCIÓN: GESTIÓN Y FEEDBACK ───────────────────── */}
          <Section title="Feedback y Bienvenida" icon="bi-stars" iconColor="bg-pink-500/10 text-pink-400" defaultOpen={suggestions.pending > 0}>
