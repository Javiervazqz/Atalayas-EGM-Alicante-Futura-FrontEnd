        ...
        <div className="flex min-h-screen bg-[#f5f5f7]" style={{ fontFamily: "'-apple-system', sans-serif" }}>
            ...
            <main className="flex-1 p-10 overflow-auto">
                ...
                <div className="flex justify-between items-end mb-10">
                    <div>
                        <h1 className="text-3xl font-bold text-[#1d1d1f] tracking-tight mb-2">Formación de Empresa</h1>
                        <p className="text-[#86868b] text-base">Supervisa el progreso de tus empleados en los cursos.</p>
                    </div>
                    <Link href="/dashboard/admin/reports" className="text-[#0071e3] text-sm font-medium hover:underline">
                    ...
                    {loading ? [1, 2, 3, 4].map(i => <div key={i} className="h-64 bg-white rounded-2xl animate-pulse border border-gray-200" />)
                        : filtered.map((course) => (
                            <div key={course.id} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-200 flex flex-col justify-between group hover:shadow-md transition-all">
                                ...
                                <Link href={`/dashboard/admin/courses/${course.id}`} className="w-full py-2.5 bg-[#0071e3] text-white text-sm font-medium rounded-xl text-center hover:bg-[#0077ed] transition-colors shadow-sm">
                                    Ver Estadísticas
                                </Link>
                            </div>
                        ))}
