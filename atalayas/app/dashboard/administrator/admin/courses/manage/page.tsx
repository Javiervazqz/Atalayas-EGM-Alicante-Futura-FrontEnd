...
export default function ManageCourses() {
  ...
  try {
    ...
    // Sincronizamos con tu DTO de NestJS
    data.append("title", courseToEdit.title);
    data.append("category", courseToEdit.category);
    
    // IMPORTANTE: Tu backend espera 'file' según el FileInterceptor('file')
    if (courseToEdit.newImageFile) {
      data.append("file", courseToEdit.newImageFile); 
    }

    const res = await fetch(API_ROUTES.COURSES.UPDATE(courseToEdit.id), {
      method: "PATCH", // Cambiado de PUT a PATCH según tu decorador
      headers: { 
        Authorization: `Bearer ${token}` 
        // No añadimos Content-Type, el navegador pone multipart/form-data solo
      },
      body: data,
    });
              ...
              onChange={(e) => {
                ...
                if (file) {
                  setCourseToEdit({
                    ...courseToEdit, 
                    newImageFile: file, 
                    imageUrl: URL.createObjectURL(file) // Para ver la previa al instante
                  });
                }
              }}
            />
