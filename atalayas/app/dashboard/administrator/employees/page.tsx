...
export default function EmployeesPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(usersData)) {
          // Filtro de seguridad: normalizamos la comparación de IDs
          if (storedUser.role === 'ADMIN') {
            ...
            setUsers(usersData); // GENERAL_ADMIN ve todo
          }
        }
      } catch (err) {
  ...
  // FILTRO POR EMPRESA + BÚSQUEDA + ORDEN
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200 mb-6 flex flex-col md:flex-row gap-4 justify-between items-center">
          <div className="relative w-full md:w-96">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-[#f5f5f7] border border-transparent focus:border-[#0071e3] focus:bg-white rounded-xl text-sm outline-none transition-all" 
            />
          </div>
          ...
          {currentUser.role === 'GENERAL_ADMIN' && (
            <select 
              ...
              className="w-full md:w-64 px-4 py-2.5 bg-[#f5f5f7] border border-transparent rounded-xl text-sm outline-none cursor-pointer"
            >
