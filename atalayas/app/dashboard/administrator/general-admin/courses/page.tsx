        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
          <div>
            ...
            <p className="text-[#86868b] mt-1 text-lg">
              Configura la oferta formativa global del sistema.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {loading ? (
            [1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="h-64 bg-white rounded-2xl animate-pulse border border-gray-200"
              />
            ))
          ) : filtered.length === 0 ? (
            ...
            filtered.map((course) => (
              <div
                key={course.id}
                className="bg-white rounded-2xl p-5 shadow-sm border border-gray-200 flex flex-col justify-between group hover:shadow-md transition-all"
              >
                <div>
                  <div className="flex justify-between items-start mb-4">
                    <span
                      className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase ${activeTab === "BASICO" ? "bg-blue-50 text-blue-600" : "bg-purple-50 text-purple-600"}`}
                    >
                      {activeTab === "BASICO"
                        ? "Onboarding"
                        : "Especialización"}
                  ...
                  <div
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl mb-4 ${activeTab === "BASICO" ? "bg-blue-100" : "bg-purple-100"}`}
                  >
                    {activeTab === "BASICO" ? "📖" : "🎓"}
                  </div>
